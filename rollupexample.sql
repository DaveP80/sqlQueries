...
-- Example Query
select null as "month", null as "request_status", sum(distance) total_distance, sum(cost) total_cost
from rides
union
select extract(month from request_date) as "month", request_status, sum(distance) total_distance, sum(cost) total_cost
from rides group by 1, request_status
union
select extract(month from request_date) as "month", null as "request_status", sum(distance) total_distance, sum(cost) total_cost
from rides group by 1 order by 1,2;
-- with ROLLUP
SELECT
...
FROM
    rides
GROUP BY
    ROLLUP (extract(month from request_date), request_status) order by 1,2;
